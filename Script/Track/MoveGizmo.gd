...
func get_proj_point(cam: Camera, p: Vector2) -> Vector3:
	   var dist = global_transform.origin.distance_to(cam.global_transform.origin)
	   var n = cam.project_ray_normal(p)
	   return cam.global_transform.origin + n * dist
